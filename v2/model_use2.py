...
def rescale_to_100(raw_score, min_observed=3, max_observed=15):
    """
    Rescale raw probability score to 1-100 range for better interpretability
    
    Parameters:
    - raw_score: Raw probability * 100 (typically 3-15 for low claim rate data)
    - min_observed: Minimum score in training data (default 3)
    - max_observed: Maximum score in training data (default 15)
    
    Returns:
    - Rescaled score from 1-100
    """
    if raw_score <= min_observed:
        return 1
    if raw_score >= max_observed:
        return 100
    
    # Linear rescaling
    rescaled = ((raw_score - min_observed) / (max_observed - min_observed)) * 99 + 1
    return int(np.clip(rescaled, 1, 100))
...
def get_age_risk_score(age_category):
    ...
    input_df = pd.DataFrame([[age_category]], columns=['agecat'])
    prob = age_model.predict_proba(input_df)[0, 1]
    raw_score = prob * 100
    return rescale_to_100(raw_score)
...
def get_vehicle_risk_score(vehicle_value, vehicle_age, vehicle_body):
    ...
    try:
        ...
        print(f"Unknown vehicle body type: {vehicle_body}")
        print(f"Valid types: {list(le_body.classes_)}")
        return None
    
    # Create DataFrame with feature names
    input_df = pd.DataFrame([[vehicle_value, vehicle_age, body_encoded]], 
                           columns=['veh_value', 'veh_age', 'veh_body_encoded'])
    prob = vehicle_model.predict_proba(input_df)[0, 1]
    raw_score = prob * 100
    return rescale_to_100(raw_score)
...
def get_location_risk_score(area):
    ...
    # Create DataFrame with feature names
    input_df = pd.DataFrame([[area_encoded]], columns=['area_encoded'])
    prob = location_model.predict_proba(input_df)[0, 1]
    raw_score = prob * 100
    return rescale_to_100(raw_score)
...
def get_gender_risk_score(gender):
    ...
    # Create DataFrame with feature names
    input_df = pd.DataFrame([[gender_encoded]], columns=['gender_encoded'])
    prob = gender_model.predict_proba(input_df)[0, 1]
    raw_score = prob * 100
    return rescale_to_100(raw_score)
...
def display_risk_scores(scores):
    ...
    # Risk interpretation (no premium tiers)
...
if __name__ == "__main__":
    ...
    print("="*60)
    print(f"\nAge 18 (category 1) -> Risk Score: {get_age_risk_score(5)}/100")
